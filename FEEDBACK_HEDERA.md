...
5. **Hono `getBody` is async** — fixed in Warrant: `resolveChallenge` awaits `getBody()` and hashes with `bodyHashFromRaw` (same helper as `warrant.fetch`). Keep client body as a UTF-8 JSON string so parse→stringify round-trips match.
