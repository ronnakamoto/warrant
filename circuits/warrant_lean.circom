...
// Child scope bits must be a subset of parent (uint64 bitmask).
...
// LeanIMT membership + D-hop attenuation. No EdDSA (WP2).
// Public: merkleRoot, contextHash, nullifier, effectiveScope,
...
template WarrantLean(D, MAX_DEPTH) {
    ...
    for (var i = 0; i < D; i++) {
        // enabled is boolean; once disabled, stay disabled
    ...
    // Mux accumulators: last enabled hop wins (split-signal form, not quadratic mux)
    signal accScope[D];
    signal accBudget[D];
    signal accExpiry[D];
    accScope[0] <== scopes[0];
    ...
    // Bind requestHash into the constraint system (leaf EdDSA added in WP2)
