...
- **Dashboard** (Next.js): the principal's private view of their tree (built from local mandates, never uploaded), per-context spend, and a red **Revoke** button. The verifier's public view: a log of `(nullifier, scope, tier, paid)` — visibly nameless.
